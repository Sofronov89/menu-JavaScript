...
redesigne = function() {
    ...
    var countAmount = Math.floor((widthWindow - 70) / widthList);

    while ((countAmount < $('div.bar li').length) && (($('div.bar li').length)>1)) {
    ...
    while ((countAmount > $('div.bar li').length) && ($('div.mobile-tab li').length)>0) {
        if ($('div.bar li:last').hasClass('active')) {
            var arW = arrBar.pop();
